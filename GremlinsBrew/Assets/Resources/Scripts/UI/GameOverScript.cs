    ...
    void MenuControls()
    {
        if (Delay == 0)
        {
            if (InputManager.Devices.Count > 0)
            {
                for (int i = 0; i < InputManager.Devices.Count; i++)
                {
                    if ((i == 0 && Input.GetKeyDown("s")) || (InputManager.Devices[i].DPadRight.WasPressed) || (x[i] <= -0.5 && (JoyStickMenuDelay[i] >= MenuDelayMax[i] || FirstJoystickMove[i] == true)))
                    {
                        UnhighlightButtons();
                    ...
                    else if ((i == 0 && Input.GetKeyDown("w")) || (InputManager.Devices[i].DPadLeft.WasPressed) || (x[i] >= 0.5 && (JoyStickMenuDelay[i] >= MenuDelayMax[i] || FirstJoystickMove[i] == true)))
                    {
                        UnhighlightButtons();
                        FirstJoystickMove[i] = false;
                        JoyStickMenuDelay[i] = 0;
